class PlainData:
    ...
    def __call__(self, *args, **kwargs):
        ...
        assert not args, \
            f'{self.wrapped_type.__name__} requires keyword argument(s) only'
        assert set(kwargs.keys()).issubset(set(self.wrapped_type_attributes.keys())), \
        ...
        # Set `__is_frozen` to True to make instance immutable
        setattr(instance, '__is_frozen', True)

        return instance

    @staticmethod
    def _extract_annotated_attributes(wrapped_type):
        return {} if not getattr(wrapped_type, '__annotations__', None) \
    ...
    def _override_str_method(wrapped_type):
        wrapped_type.__str__ = lambda self: f'<{wrapped_type.__name__}: ' + ', '.join(
