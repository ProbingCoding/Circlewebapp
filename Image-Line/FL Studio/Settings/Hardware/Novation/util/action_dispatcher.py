class ActionDispatcher:
    ...
    def dispatch(self, action):
        ...
        callback_list = self._callback_lists_for_actions.get(action_name, None)
    ...
    def _extract_callbacks(subscriber):
        return {attr_name.lstrip('handle_'): getattr(subscriber, attr_name) for attr_name in dir(subscriber) if
                ActionDispatcher._is_action_callback(subscriber, attr_name)}

    @staticmethod
    def _is_action_callback(subscriber, attr_name):
        return attr_name.startswith('handle_') \
