...
class KeyboardControllerScreenWriter:
    ...
    def display_parameter_name(self, control_index, name):
        if (pot_index := self.product_defs.ControlIndexToPotIndex.get(control_index)) is not None:
            ...
            screen_control_index = self.fader_screen_index_offset + self.product_defs.ControlIndexToFaderIndex[
                control_index]

        set_parameter_name_message = self.set_parameter_name_message_header + [screen_control_index] + list(name.encode(
    ...
    def display_parameter_value(self, control_index, value):
        ...
        set_parameter_value_message = self.set_parameter_value_message_header + [screen_control_index] + list(
            value.encode(
                encoding="ascii", errors="replace"))
