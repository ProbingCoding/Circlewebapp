from script.device_dependent.FLkey import MixerPanPotLayoutManager, MixerVolumePotLayoutManager
from script.device_dependent.Launchkey import VolumeFaderLayoutManager
...
from script.device_independent.view import ChannelSelectNameHighlightView, ChannelSelectView, ChannelSelectedScreenView, \
    MetronomeButtonView, TransportPlayPauseButtonView, TransportRecordButtonView, TransportStopButtonView, \
    UndoButtonView
from script.model import Model


class Application:
    def __init__(self, pad_led_writer, button_led_writer, fl, action_dispatcher, screen_writer, product_defs,
                 device_manager):
    ...
    def _create_pad_layout_manager(self, layout):
        if layout == self.product_defs.PadLayout.Drum:
            return DrumPadLayoutManager(self.action_dispatcher, self.pad_led_writer, self.button_led_writer, self.fl,
    ...
    def _create_pot_layout_manager(self, layout):
        ...
        if layout == self.product_defs.PotLayout.Pan:
            return MixerPanPotLayoutManager(self.action_dispatcher, self.button_led_writer, self.fl,
                                            self.screen_writer, self.product_defs, self.model, self.fl_window_manager)
        return None

    def _create_fader_layout_manager(self, layout):
        if layout == self.product_defs.FaderLayout.Volume:
            return VolumeFaderLayoutManager(self.action_dispatcher, self.fl, self.screen_writer, self.model,
                                            self.fl_window_manager)
        return None
