...
class ScrollingArrowButtonView(View):
    ...
    def __init__(self, action_dispatcher, button_led_writer, product_defs, *, decrement_button, increment_button,
                 last_page=0,
                 on_page_changed=None, on_page_change_attempt=None, speed=Speed.Default.value):
        super().__init__(action_dispatcher)
        self.value = ValueWithRange(upper_bound=last_page)
        self.decrement_button_view = SingleButtonView(button_led_writer, product_defs, decrement_button,
                                                      is_available=lambda: not self.value.reached_lower_bound())
        self.increment_button_view = SingleButtonView(button_led_writer, product_defs, increment_button,
                                                      is_available=lambda: not self.value.reached_upper_bound())
        self.on_page_changed = on_page_changed
        self.on_page_change_attempt = on_page_change_attempt
