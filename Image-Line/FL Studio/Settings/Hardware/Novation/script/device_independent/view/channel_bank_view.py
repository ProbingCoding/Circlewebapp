...
class ChannelBankView(View):
    ...
    def __init__(self, action_dispatcher, surface, fl, product_defs, model):
        ...
        self.arrow_button_view = ScrollingArrowButtonView(action_dispatcher, surface, product_defs,
    ...
    def handle_OnRefreshAction(self, action):
        if action.flags & RefreshFlags.ChannelGroup.value or (
                action.flags & RefreshFlags.TransportStatus.value and
                action.flags & RefreshFlags.ChannelSelection.value and
                action.flags & RefreshFlags.RemoteLinks.value):
            number_of_banks = self._calculate_number_of_banks()
            self.arrow_button_view.set_page_range(first_page=0, last_page=number_of_banks - 1,
                                                  notify_on_page_change=True)
