...
class ChannelRackDrumPadsView(View):
    channel_offset_for_pad = [8, 9, 10, 11, 12, 13, 14, 15,
                              0, 1, 2, 3, 4, 5, 6, 7]
    ...
    def handle_OnRefreshAction(self, action):
        if action.flags & RefreshFlags.ChannelSelection.value or \
                action.flags & RefreshFlags.LedUpdate.value and not self.fl.is_any_channel_selected():
    ...
    def _update_pad_led(self, pad):
        ...
        if pad in self.global_channel_for_held_pad:
            ...
            r, g, b = self.fl.get_channel_colour(group_channel=group_channel)
            self.pad_led_writer.set_pad_colour(pad, (r // 2, g // 2, b // 2))
