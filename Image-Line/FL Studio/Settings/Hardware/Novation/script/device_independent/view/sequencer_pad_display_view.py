...
class SequencerPadDisplayView(View):
    ...
    colour_for_step_edit_parameter_index = {
        StepEditParameters.Pitch.value.index: [Colours.step_pitch_step_latched, Colours.step_pitch_step_on,
                                               Colours.step_pitch_step_off],
        StepEditParameters.Velocity.value.index: [Colours.step_velocity_step_latched, Colours.step_velocity_step_on,
                                                  Colours.step_velocity_step_off],
        StepEditParameters.Release.value.index: [Colours.step_release_step_latched, Colours.step_release_step_on,
                                                 Colours.step_release_step_off],
        StepEditParameters.PitchFine.value.index: [Colours.step_pitch_fine_step_latched,
                                                   Colours.step_pitch_fine_step_on,
                                                   Colours.step_pitch_fine_step_off],
        StepEditParameters.Pan.value.index: [Colours.step_pan_step_latched, Colours.step_pan_step_on,
                                             Colours.step_pan_step_off],
        StepEditParameters.ModX.value.index: [Colours.step_mod_x_step_latched, Colours.step_mod_x_step_on,
                                              Colours.step_mod_x_step_off],
        StepEditParameters.ModY.value.index: [Colours.step_mod_y_step_latched, Colours.step_mod_y_step_on,
                                              Colours.step_mod_y_step_off],
        StepEditParameters.Shift.value.index: [Colours.step_shift_step_latched, Colours.step_shift_step_on,
    ...
    def _channel_colour(self):
        r, g, b = self.fl.get_channel_colour()
        brightness = max(r, g, b)

        if brightness < self.min_channel_colour_brightness:
            target = self.min_channel_colour_brightness
    ...
    def _dim_channel_colour(self):
        return tuple(component // 8 for component in self._channel_colour())

    def _bright_channel_colour(self):
        return tuple(component // 2 for component in self._channel_colour())
    ...
    def _update_all_leds(self):
        for pad in range(0, Pads.Num.value):
            self._update_led_for_pad(pad)
