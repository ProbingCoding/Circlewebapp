...
class SlicerPluginView(View):
    note_offset_for_pad = [4, 5, 6, 7, 8, 9, 10, 11,
                           -4, -3, -2, -1, 0, 1, 2, 3]
    ...
    def _note_is_in_range(self, note):
        return SlicexNoteRange.Min <= note <= SlicexNoteRange.Max \
               and self.fl.plugin.get_note_name(self.fl.selected_channel(), note)
